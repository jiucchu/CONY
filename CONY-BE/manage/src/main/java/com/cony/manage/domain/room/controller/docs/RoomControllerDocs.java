...
public interface RoomControllerDocs {
        ...
        @Operation(summary = "방 이름 수정", description = "방의 이름을 수정합니다. (방 생성자 권한 필요)")
        @ApiResponses(value = {
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "수정 성공"),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "수정 권한 없음 (방 생성자가 아님)", content = @Content),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "방을 찾을 수 없음", content = @Content)
        })
        ApiResponse<Void> updateRoomName(
                        @Parameter(description = "방 ID", required = true) @PathVariable("roomId") Long roomId,
                        @RequestBody(description = "변경할 방 이름", required = true) RoomUpdateRequestDto requestDto,
                        @AuthUser Long userId);
}
